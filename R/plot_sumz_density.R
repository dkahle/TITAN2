...
function  (titan.out, filter = TRUE, sumz=TRUE,points=FALSE, ribbon=TRUE, density=TRUE, change_points=TRUE,
             sumz1 = TRUE, sumz2 = TRUE, xmin = min(titan.out$env), xmax = max(titan.out$envcls),
             xlabel = "Environmental Gradient", y1label = NULL, y2label="Density", alpha1=0.65, alpha2=0.5,
             col1 = "steelblue4", col2 = "red", axis_lab_size=20, legend.position = c(.9, .9), ...)
{
   if (filter) {
    ivz <- titan.out$ivz.f
...
maxFsumz=data.frame(titan.out$maxFsumz)
 ...
 if(density) {
      ...
      xlim(xmin, xmax)+
      theme_classic(base_size=axis_lab_size)+
      theme(axis.text.x = (if(sumz) element_blank() else element_text()),
            plot.margin = structure(
 ...
 if(density){
    if(change_points){
        if(sumz){plot_grid(ptop,pmid,pbot, ncol = 1, rel_heights=c(2,3,6), align = "v")
      }else{
        plot_grid(ptop,pmid, ncol = 1, rel_heights=c(2,3), align = "v")}
    }else{
        if(sumz){plot_grid(pmid, pbot, ncol = 1, rel_heights=c(3,6), align = "v")
      }else{
        plot_grid(pmid, ncol = 1)}
    }}else{
    if(sumz){
      if(change_points){plot_grid(ptop, pbot, ncol = 1, rel_heights=c(2,6), align = "v")}else{plot_grid(pbot)}
    }
