...
`plotTaxa()`.  We have replaced this plot with the distributions in
...
of the graph, so see the full documentation with `?plotTaxa`, but you can get
started by simply calling `plotTaxa()` on the output of `titan()`:
...
The default graphic from `plotCPs()` is a $z$-score weighted probability density
...
Finally, if `taxa.dist = FALSE`, instead of a taxon-specific plot, `plotCPs()`
