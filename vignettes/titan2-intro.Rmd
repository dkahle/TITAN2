...
We are now ready to run the `titan()` function.  (Note: this takes a while to 
run, so beware.)
...
replicates on the `obsiv.prob`; reliability, when coupled with purity, is a
...
The default graphic from `plot_cps()` is a $z$-score weighted probability
